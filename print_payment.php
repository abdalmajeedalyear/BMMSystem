 ...
 * تدعم الأرقام من 0 إلى 999,999,999 مع مراعاة قواعد اللغة العربية
 */
function numberToWords($number) {
    // التأكد من أن الرقم رقمي
    if (!is_numeric($number)) return 'ريال يمني فقط لا غير';
    
    // نأخذ الجزء الصحيح فقط (بدون كسور)
    ...
    // معالجة الباقي (أقل من ألف)
    if ($number > 0) {
        // إضافة "و" إذا كان هناك أجزاء سابقة (آلاف أو ملايين)
...
function convertLessThanOneThousand($number, $ones, $tens, $hundreds) {
    ...
    if ($number >= 100) {
        ...
        // إذا كان هناك أجزاء بعد المئات (عشرات أو آحاد) نضيف "و"
        if ($number > 0) {
            $result .= ' و ';
        } else {
            $result .= ' ';
...
<head>
    ...
    <title>طباعة سند قبض - <?php echo $payment['payment_number']; ?></title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="print-container">
        ...
        <div class="print-body">
            <div class="company-infon">
                ...
                <div class="payment-number">رقم السند: <?php echo $payment['payment_number']; ?></div>
                <div class="payment-number">تاريخ السند: <?php echo $payment['payment_date']; ?></div>
            </div>
            ...
            <table class="info-table">
                ...
                <tr>
                        ...
                        <td class="value"><?php echo $payment['customer_name']; ?></td>
                    
                    
                        <td class="label">رقم الهاتف:</td>
                        <td class="value"><?php echo $payment['customer_phone'] ?: '---'; ?></td>
                
                </tr>
                `
                <tr>
                    <td class="label">طريقة الدفع:</td>
                    <td class="value"><?php echo $payment['payment_method']; ?></td>
                </tr>
                <?php if ($payment['reference_number']): ?>
                <tr>
                    <td class="label">رقم المرجع:</td>
                    <td class="value"><?php echo $payment['reference_number']; ?></td>
                </tr>
                <?php endif; ?>
                <?php if ($payment['notes']): ?>
                <tr>
                    <td class="label">ملاحظات:</td>
                    <td class="value"><?php echo $payment['notes']; ?></td>
                </tr>
                <?php endif; ?>
            </table>
            
            <div class="amount-box">
                <div class="amount"><?php echo formatNumber($payment['payment_amount']); ?> ر.ي</div>
        ...
        <div class="footer">
           شكراً لتعاملكم - نظام إدارة المخازن |تم الإنشاء بواسطة:  <?php echo $payment['created_by_name'] ?? 'غير محدد'; ?> | تم الطباعة بواسطة: <?php echo $_SESSION['full_name'] ?? 'غير محدد'; ?> | جميع الحقوق محفوظة
        </div>
    </div>
