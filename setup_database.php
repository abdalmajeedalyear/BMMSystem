...
if ($conn->query($sql_create_table) === TRUE) {
    echo "جدول store_items تم إنشاؤه بنجاح أو كان موجوداً بالفعل<br>";
} else {
...
$sql_insert_data = "
INSERT IGNORE INTO `store_items` (`item_code`, `item_name`, `category`, `quantity`, `unit`, `cost_price`, `selling_price`, `min_stock_level`, `description`) VALUES
...
if ($conn->query($sql_insert_data) === TRUE) {
    echo "تم إدخال البيانات التجريبية بنجاح<br>";
} else {
    echo "خطأ في إدخال البيانات: " . $conn->error . "<br>";
}

// التحقق من عدد السجلات
$result = $conn->query("SELECT COUNT(*) as count FROM store_items");
if ($result) {
    $row = $result->fetch_assoc();
    echo "عدد الأصناف في الجدول: " . $row['count'] . "<br>";
}

echo "<br><a href='Store_items.php'>الانتقال إلى صفحة المخزون</a>";
?>