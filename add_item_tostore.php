...
function generatePurchaseNumber($conn) {
    ...
    // في جدول purchases العمود الأساسي اسمه id وليس purchase_id
    $sql = "SELECT MAX(id) as max_id FROM purchases";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_purchase'])) {
        ...
        foreach ($_POST['products'] as $product) {
            if (!empty($product['product_name']) && !empty($product['product_quantity']) && $product['product_quantity'] > 0) {
                ...
                $products_to_save[] = [
                    ...
                    'item_id' => intval($product['item_id'] ?? 0),
    ...
    try {
        ...
        $supplier_name = $conn->real_escape_string($_POST['supplier_name'] ?? 'مورد نقدي');
        ...
        // ==================== إضافة/تحديث المورد في جدول الموردين ====================
        $supplier_id = 0;
        if (!empty($supplier_name) && $supplier_name != 'مورد نقدي') {
            // التحقق من وجود المورد مسبقاً
            $check_supplier = $conn->query("SELECT supplier_id FROM suppliers WHERE supplier_name = '$supplier_name'");
            
            if ($check_supplier && $check_supplier->num_rows > 0) {
                // المورد موجود - الحصول على معرفه
                $supplier_row = $check_supplier->fetch_assoc();
                $supplier_id = $supplier_row['supplier_id'];
                
                // تحديث رقم الهاتف إذا كان المدخل مختلفاً
                ...
                // إضافة مورد جديد
                $insert_supplier = "INSERT INTO suppliers 
                                    (supplier_name, supplier_phone, created_at) 
                                    VALUES ('$supplier_name', '$supplier_phone', NOW())";
                
                if ($conn->query($insert_supplier)) {
                    $supplier_id = $conn->insert_id;
                } else {
                    // سجل الخطأ ولكن لا توقف المعاملة
                    error_log("خطأ في إضافة المورد: " . $conn->error);
                }   
            }
        }
        ...
        // إدخال فاتورة المشتريات مع الحقول الجديدة
        // إدخال فاتورة المشتريات مع الحقول الجديدة
        $stmt_purchase = $conn->prepare("INSERT INTO purchases 
            (purchase_number, purchase_date, supplier_name, supplier_id, supplier_phone, subtotal, total_discount, grand_total, paid_amount, remaining_amount, payment_method, payment_status, notes, created_by) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmt_purchase->bind_param(
            "sssisddddsdssi",  // 14 حرفاً (لأن لدينا 14 متغيراً)
            $purchase_number,   // s (string)
            $purchase_date,     // s (string)
            $supplier_name,     // s (string)
            $supplier_id,       // i (integer)
            $supplier_phone,    // s (string)
            $subtotal,          // d (double)
            $total_discount,    // d (double)
            $grand_total,       // d (double)
            $paid_amount,       // d (double)
            $remaining_amount,  // d (double)
        ...
        if (!$stmt_purchase->execute()) {
            throw new Exception("خطأ في حفظ فاتورة المشتريات: " . $stmt_purchase->error);
        }
        
        $purchase_id = $conn->insert_id;
        
        // إدخال أصناف المشتريات وتحديث المخزون
        $added_count = 0;
        $new_count = 0;
        
        foreach ($products_to_save as $product) {
            
            // إدخال عنصر المشتريات - بدون product_id
            ...
            if (!empty($product['item_id'])) {
                ...
                $update = "UPDATE products SET 
                          current_quantity = current_quantity + {$product['qty']},
                ...
                $check = $conn->query("SELECT product_id FROM products WHERE product_name = '{$product['name']}' AND product_unit = '{$product['unit']}'");
                ...
                $added_count++;
                    ...
                    // المنتج غير موجود (إما الاسم مختلف أو الوحدة مختلفة) - إنشاء منتج جديد
                    $unique_code = "BM-" . date('Y') . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT);
                    ...
                    while (true) {
                        ...
                        $unique_code = "BM-" . date('Y') . str_pad(rand(1, 9999) + $counter, 4, '0', STR_PAD_LEFT);
                        $counter++;
                    }
                    
                    $selling_price = $product['price'] * 1.25; // ربح 25%
                    ...
                    $insert = "INSERT INTO products 
                            ...
                            VALUES ('$unique_code', '{$product['name']}', 'مواد متنوعة', {$product['qty']}, '{$product['unit']}', {$product['price']}, $selling_price, 10, NOW())";
                    
                    if (!$conn->query($insert)) {
                        throw new Exception("خطأ في إضافة منتج جديد");
                    }
                    $new_count++;
                }
            }
        ...
        $message = "✅ تم حفظ فاتورة المشتريات رقم <strong>$purchase_number</strong> بنجاح! ";
        if ($added_count > 0) $message .= "تم تحديث $added_count منتج موجود. ";
        if ($new_count > 0) $message .= "تم إضافة $new_count منتج جديد.";
        
        // إضافة رسالة عن المورد إذا تمت إضافته
        if ($supplier_id > 0) {
            $message .= " تم إضافة المورد إلى قاعدة البيانات.";
        }
...
<head>
    ...
    <title>فاتورة مشتريات جديدة - نظام المخازن</title>
    ...
    <style>
        ...
        th { background: #f0f0f0; padding: 12px; text-align: center; border: 1px solid #ddd; }
        ...
        .header-item label { font-size: 12px; color: #666; margin-bottom: 5px; font-weight: bold; }
        .header-item input, .header-item select { background: white; border: 1px solid #ddd; border-radius: 4px; padding: 10px; font-size: 14px; }
        .header-item input[readonly] { background: #f5f5f5; color: #666; }
        ...
        .shortcuts span { background: #eee; padding: 3px 6px; border-radius: 4px; margin: 0 5px; font-size: 12px; }
        
        .summary-section { margin-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        .totals { background: #f0f8ff; padding: 20px; border: 1px solid #2196F3; border-radius: 8px; }
        
        .delete-btn { background: none; border: none; color: #f44336; cursor: pointer; font-size: 16px; }
        
        .autocomplete-wrapper { position: relative; }
        .autocomplete-dropdown {
            position: absolute; top: 100%; left: 0; right: 0; background: white; border: 1px solid #ddd;
            border-top: none; border-radius: 0 0 4px 4px; max-height: 200px; overflow-y: auto;
        ...
        .autocomplete-item .item-name { font-weight: bold; color: #333; }
        .autocomplete-item .item-code { font-size: 11px; color: #666; }
        .autocomplete-item .item-details { display: flex; justify-content: space-between; font-size: 11px; margin-top: 5px; }
        .autocomplete-item .item-price { color: #4CAF50; }
        .autocomplete-item .item-stock { color: #ff9800; }
        
        .footer-actions { display: flex; justify-content: space-between; align-items: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #ddd; }
        
        .alert {
            position: fixed; top: 20px; right: 20px; padding: 15px 25px; border-radius: 4px;
            color: white; font-weight: bold; z-index: 9999; animation: slideIn 0.3s ease;
        }
...
<?php endif; ?>
<div class="head_link">
    <a href="purchases_page.php" class="back-link">
...
<div class="container">
    ...
    <h1><i class="fas fa-shopping-cart" style="color: #2196F3; margin-left: 10px;"></i> فاتورة مشتريات جديدة</h1>
    ...
    <form method="post" id="purchaseForm">
        ...
        <div class="invoice-header">
            ...
            <div class="header-item">
                ...
                <input type="text" name="purchase_number" value="<?php echo $next_purchase_number; ?>" readonly>
            </div>
            
            <!-- التاريخ -->
            <div class="header-item">
                <label>تاريخ الفاتورة</label>
                <input type="date" name="purchase_date" value="<?php echo $today_date; ?>" required>
            </div>
            ...
            <div class="header-item col-span-2">
                ...
                <div style="position: relative;">
                    <input type="text" name="supplier_name" placeholder="أدخل اسم المورد" value="مورد نقدي" required>
                    <i class="fas fa-truck" style="position: absolute; left: 10px; top: 12px; color: #999;"></i>
                </div>
            </div>
            ...
            <div class="header-item col-span-2">
                ...
                <div style="position: relative;">
                    <input type="text" name="supplier_phone" placeholder="أدخل رقم هاتف المورد">
            ...
            <div class="header-item">
                ...
                <select name="payment_status">
                    ...
                    <option value="جزئي">مدفوعة جزئياً</option>
                        ...
                        <!-- ===== الحقول الجديدة: المبلغ المسلم والمتبقي للمورد ===== -->
            <!-- المبلغ المسلم -->
            <div class="header-item">
                <label>المبلغ المسلم للمورد</label>
                <input type="number" name="paid_amount" id="paid_amount" value="0" min="0" step="0.01" class="paid-field" oninput="updatePaymentInfo()" style="border: 2px solid #4CAF50;">
            </div>

            <!-- المبلغ المتبقي -->
            <div class="header-item">
                <label>المبلغ المتبقي</label>
                <input type="number" name="remaining_amount" id="remaining_amount" value="0" min="0" step="0.01" class="remaining-field" readonly style="background:#f0f0f0; border: 2px solid #f44336; font-weight: bold;">
            </div>
        </div>
        ...
        <table>
            ...
            <tbody id="productsTableBody">
                ...
                <tr id="row_0">
                    ...
                    <td>
                        <!-- <input type="number" name="products[0][product_quantity]" id="product_quantity_0" value="1" min="1" oninput="calculateRow(0)"> -->
        ...
        <div class="summary-section">
            ...
            <div class="totals">
                ...
                <div style="margin-top:15px; padding:10px; background:#e8f5e9; border-radius:4px; font-size:13px; color:#2e7d32;">
                    <i class="fas fa-info-circle"></i> سيتم إضافة المنتجات للمخزون تلقائياً
                </div>
            </div>
        </div>
        ...
        <div class="footer-actions">
            ...
            <div>
                <button type="button" onclick="window.location.href='Store_items.php'" class="btn btn-warning" style="margin-left:10px;">
...
<script>
    ...
    function showSuggestions(products, rowId) {
        ...
        products.forEach(product => {
            html += `
                <div class="autocomplete-item" onclick="selectProduct(${rowId}, ${product.id}, '${product.name.replace(/'/g, "\\'")}', '${product.unit}', ${product.price})">
                    <div class="item-name">${product.name}</div>
                    <div class="item-code">كود: ${product.code} | الوحدة: ${product.unit}</div>
                    <div class="item-details">
                        <span class="item-price">💰 السعر: ${product.price} ر.س</span>
    ...
    function selectProduct(rowId, productId, productName, productUnit, productPrice) {
    ...
    function calculateRow(rowId) {
        const qty = parseFloat(document.getElementById(`product_quantity_${rowId}`).value) || 0;
    ...
    function updatePaymentInfo() {
        ...
        if (paidAmount >= grandTotal && grandTotal > 0) {
            paymentStatusSelect.value = 'مدفوعة';
        } else if (paidAmount > 0) {
            paymentStatusSelect.value = 'جزئي';
        } else {
            paymentStatusSelect.value = 'غير مدفوعة';
        }
    ...
    // تعديل دالة calculateTotals لاستدعاء updatePaymentInfo
